    ...
    <div className="bg-[#1e293b] rounded-2xl p-5 border border-slate-700/50 hover:border-slate-600/50 transition-colors">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-slate-400">Trip Wallet</span>
        </div>
      ...
      {!loading && !budget && (
        <div className="text-center py-4">
          <p className="text-sm text-slate-400 mb-4">No active trip wallet</p>
          <button
            onClick={onCreateWallet}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 font-medium text-sm transition-colors"
          >
      ...
      {!loading && budget !== undefined && (
        <div>
          {tripTitle && (
            <p className="text-xs text-slate-500 mb-2 truncate">{tripTitle}</p>
          )}
